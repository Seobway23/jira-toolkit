...
def github_event() -> tuple[str, str, str]:
    ...
    if action in {"opened", "closed", "edited", "synchronize", "reopened"}:
        ...
        return event, title, url
    return "updated", title, url


def gitlab_event() -> tuple[str, str, str]:
...
def main() -> None:
    ...
    if source == "github":
        event, title, url = github_event()
    else:
        event, title, url = gitlab_event()
